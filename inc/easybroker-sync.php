...
function eb_sync_properties() {
    // Prepare available API keys safely
    $api_keys = [];
    if (defined('EASYBROKER_API_KEY')) {
        $api_keys[] = EASYBROKER_API_KEY;
    }
    if (defined('EASYBROKER_API_KEY2')) {
        $api_keys[] = EASYBROKER_API_KEY2;
    }
    ...
    foreach ( $api_keys as $api_key ) {
        ...
        do {
            ...
            foreach ( $properties as $p ) {
                ...
                update_post_meta($post_id, 'eb_operation', $operation_type);
                update_post_meta($post_id, 'eb_currency', $currency);
                update_post_meta($post_id, 'eb_location', eb_safe_value($p, 'location', 'No location'));
                update_post_meta($post_id, 'eb_property_type', eb_safe_value($p, 'property_type', 'No type'));
